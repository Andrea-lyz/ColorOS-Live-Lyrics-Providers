...
internal val NETEASE_ARTWORK_BITMAP_KEYS = arrayOf(
    MediaMetadata.METADATA_KEY_ART,
    MediaMetadata.METADATA_KEY_ALBUM_ART,
    MediaMetadata.METADATA_KEY_DISPLAY_ICON
)

object NeteaseMetadataCopy {
    ...
    fun newPreservingBuilder(metadata: MediaMetadata): MediaMetadata.Builder {
        ...
        metadata.keySet().forEach { key ->
            runCatching {
                when {
                    key in NETEASE_ARTWORK_BITMAP_KEYS -> {
    ...
    fun copyWithLyricInfo(metadata: MediaMetadata, lyricInfo: String): MediaMetadata {
        return newPreservingBuilder(metadata)
            .putString(NeteasePlayerConstants.METADATA_KEY_LYRIC_INFO, lyricInfo)
            .build()
    }
