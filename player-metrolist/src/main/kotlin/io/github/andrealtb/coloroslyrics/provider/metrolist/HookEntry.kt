...
@InjectYukiHookWithXposed(modulePackageName = "io.github.andrealtb.coloroslyrics.provider.metrolist")
class HookEntry : IYukiHookXposedInit {

    override fun onHook() {
        YukiHookAPI.encase {
            MetrolistPlayerConstants.QUALIFIED_HOST_PACKAGES.forEach { packageName ->
                loadApp(packageName) {
                    initMetrolist(packageName)
                }
            }
        }
    }

    private fun PackageParam.initMetrolist(packageName: String) {
        RuntimeModeResolver.notifyXposedHookActive()
        onAppLifecycle {
            onCreate {
                val hostContext = appContext ?: return@onCreate
                MetrolistPlayerHooker(
    ...
    override fun onInit() {
        if (ProviderDebugConfig.readXposedSwitch(
                ...
                ProviderId.METROLIST
            )
        ) {
            YukiHookAPI.configs {
                debugLog {
                    tag = "MetrolistMusicProvider"
                }
            }
        }
    }
