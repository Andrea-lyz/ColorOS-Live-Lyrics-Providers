...
internal object MetrolistProviderPreferences {
    ...
    fun read(context: Context): Selection {
        ...
        if (!file.exists()) {
            Log.d(TAG, "DataStore file not found at ${file.absolutePath}, using default order")
        ...
        return try {
            ...
            Log.d(TAG, "Provider order source=$source, configured=$order")
            Selection(order, enabled, source)
        } catch (error: Exception) {
            Log.d(TAG, "Failed to parse Metrolist DataStore, using default order: ${error.message}")
