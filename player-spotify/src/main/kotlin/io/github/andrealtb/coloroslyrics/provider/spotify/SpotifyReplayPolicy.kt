...
internal data class SpotifyReplaySnapshot(
    val session: WeakReference<Any>,
    val track: TrackIdentity,
    val generation: Long,
    val publication: SpotifyPublication
)

internal object SpotifyReplayPolicy {
    ...
    fun shouldStripStale(
        incomingLyricInfo: String?,
        hostPackage: String,
        incomingTrack: TrackIdentity?,
        currentTrack: TrackIdentity?
    ): Boolean {
        if (!isModuleOwned(incomingLyricInfo, hostPackage)) return false
        if (incomingTrack == null) return true
        if (currentTrack == null) return true
        return !TrackIdentityPolicy.isSameTrack(incomingTrack, currentTrack)
    }
}
