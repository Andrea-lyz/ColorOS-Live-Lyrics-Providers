...
object SpotifyPendingPublicationPolicy {
    enum class Decision {
        PUBLISH,
        PENDING,
        DROP_STALE
...
class SpotifyPendingPublicationStore {
    private var pending: SpotifyPublication? = null

    @Synchronized
    fun replace(publication: SpotifyPublication): SpotifyPublication? {
    ...
    @Synchronized
    fun peek(): SpotifyPublication? = pending

    @Synchronized
    fun takeIfSame(expected: SpotifyPublication): Boolean {
        if (pending === expected) {
            pending = null
            return true
        }
