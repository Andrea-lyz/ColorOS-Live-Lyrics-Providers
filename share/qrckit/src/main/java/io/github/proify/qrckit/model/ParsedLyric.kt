...
data class ParsedLyric(
    ...
    val richLyricLines: List<RichLyricLine> by lazy {
        ...
        sourceLines.mapIndexed { index, line ->
            RichLyricLine(
                ...
                text = line.text,
                translation = matchedTranslations[index]
                    ?.text
                    ?.takeUnless { it.trim() == "//" },
    ...
    /**
     * Associates QQ QRC translations with their source lines as an ordered,
     * one-to-one sequence. QQ's QRC and LRC translation tracks can differ by
     * more than a few milliseconds, so matching every source line independently
     * against a fixed 50 ms threshold drops valid translations and can reuse one
     * translation for adjacent source lines.
     */
