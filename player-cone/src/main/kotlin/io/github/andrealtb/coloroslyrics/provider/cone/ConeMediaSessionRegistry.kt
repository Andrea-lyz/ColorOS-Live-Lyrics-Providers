...
class ConeMediaSessionRegistry {
    ...
    fun selectUnique(track: TrackIdentity): Any? {
        prune()
        return ConeMediaSessionSelectionPolicy.selectUnique(entries.mapNotNull { candidate ->
            candidate.session.get()?.let { session ->
                ConeSessionCandidate(
...
internal data class ConeSessionCandidate(
    val session: Any,
...
internal object ConeMediaSessionSelectionPolicy {
    fun selectUnique(candidates: List<ConeSessionCandidate>, track: TrackIdentity): Any? = candidates
        .filter { candidate ->
            !candidate.released && candidate.active &&
                ConeMediaSessionRegistry.isPlaybackStateValid(candidate.playbackState) &&
...
internal data class ConeReplaySnapshot(
    val session: WeakReference<Any>,
    val track: TrackIdentity,
    val generation: Long,
...
internal object ConeReplayPolicy {
    const val OWNED_PROVIDER_FRAGMENT = "\"provider\":\"ink.trantor.coneplayer"
    const val OWNED_SOURCE_FRAGMENT = "\"source\":\"ink.trantor.coneplayer"

    fun isModuleOwned(payload: String?): Boolean = payload != null &&
        payload.contains(OWNED_PROVIDER_FRAGMENT) &&
        payload.contains(OWNED_SOURCE_FRAGMENT)

    fun shouldReplay(
        cached: ConeReplaySnapshot?,
        selectedSession: Any?,
        ...
        if (!TrackIdentityPolicy.isSameTrack(cached.track, incomingTrack) ||
            !TrackIdentityPolicy.isSameTrack(cached.track, currentTrack)
        ) return false
