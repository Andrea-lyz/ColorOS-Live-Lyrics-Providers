...
class LxNativePublisherTest {
    @Test
    fun failedPlatformSessionCommitIsReported() {
        assertEquals(
            NativeLyricInfoPublisher.Result.COMMIT_FAILED,
            LxNativePublisher.classifyCommit(NativeLyricInfoPublisher.Result.PUBLISHED, false)
        )
    }
    ...
    fun rejectedCoreResultIsPreserved() {
        assertEquals(
            ...
            LxNativePublisher.classifyCommit(NativeLyricInfoPublisher.Result.PAYLOAD_TOO_LARGE, false)
        )
    }
}
